#pragma once

#include "vector.h"

#include <memory>

namespace vector {

const int8_t START_MEMORY_ALLOCATE = 10;

template <typename T> Vector<T>::Vector() : sz_(0), cap_(0), arr_(nullptr, Deleter) {}

template <typename T>
Vector<T>::Vector(std::initializer_list<T> init)
    : sz_(init.size()), cap_(init.size()) {
  if (sz_ > 0) {
  arr_ = std::shared_ptr<T[]>(new T[init.size()], Deleter);
  size_t i = 0;
  for (const T &fig : init) {
    arr_[i] = fig;
    ++i;
  }
  } else {
    arr_ = std::shared_ptr<T[]>(nullptr, Deleter);
  }
}

template <typename T> T &Vector<T>::operator[](size_t pos) {
  if (pos >= sz_) {
    throw exceptions::InvalidIndex("Invalid index");
  }
  return arr_[pos];
}

template <typename T> const T &Vector<T>::operator[](size_t pos) const {
  if (pos >= sz_) {
    throw exceptions::InvalidIndex("Invalid index");
  }
  return arr_[pos];
}

template <typename T> T &Vector<T>::Front() const noexcept {
  if (sz_ == 0) {
    throw exceptions::VectorIsNull("Vector is null!");
  }
  return arr_[0];
}

template <typename T> bool Vector<T>::IsEmpty() const noexcept {
  return sz_ == 0;
}

template <typename T> T &Vector<T>::Back() const noexcept {
  if (sz_ == 0) {
    throw exceptions::VectorIsNull("Vector is null!");
  }
  return arr_[sz_ - 1];
}

template <typename T> T* Vector<T>::Data() const noexcept { return arr_.get(); }

template <typename T> size_t Vector<T>::Size() const noexcept { return sz_; }

template <typename T> size_t Vector<T>::Capacity() const noexcept {
  return cap_;
}

template <typename T> void Vector<T>::Reserve(size_t new_cap) {
  if (new_cap <= cap_) {
    return;
  }
  auto new_arr = std::shared_ptr<T[]>(new T[new_cap], Deleter);
  for (size_t i = 0; i < sz_; i++) {
    new_arr[i] = std::move(arr_[i]);
  }
  arr_ = std::move(new_arr);
  cap_ = new_cap;
}

template <typename T> void Vector<T>::Clear() noexcept { sz_ = 0; }

template <typename T> void Vector<T>::Insert(size_t pos, T value) {
  if (pos > sz_) {
    return;
  }
  if (cap_ == sz_) {
    Reserve(cap_ == 0 ? START_MEMORY_ALLOCATE : cap_ * 2);
  }
  for (size_t i = sz_; i > pos; --i) {
    arr_[i] = std::move(arr_[i - 1]);
  }
  arr_[pos] = std::move(value);
  ++sz_;
}

template <typename T> void Vector<T>::Erase(size_t begin_pos, size_t end_pos) {
  if (end_pos > sz_) {
    return;
  }
  for (size_t i = end_pos; i < sz_; ++i) {
    arr_[i - (end_pos - begin_pos)] = std::move(arr_[i]);
  }
  sz_ -= (end_pos - begin_pos);
}

template <typename T> void Vector<T>::Erase(size_t pos) { Erase(pos, pos + 1); }

template <typename T> void Vector<T>::PushBack(T value) {
  if (cap_ == 0) {
    Reserve(START_MEMORY_ALLOCATE);
  }
  if (cap_ == sz_) {
    Reserve(2 * cap_);
  }
  arr_[sz_] = std::move(value);
  ++sz_;
}

template <typename T> void Vector<T>::PopBack() {
  if (sz_ == 0)
    return;
  --sz_;
}

template <typename T>
template <typename U>
double Vector<T>::TotalArea() const {
  double total = 0.0;
  for (size_t i = 0; i < sz_; ++i) {
    total += arr_[i]->Area();
  }
  return total;
}

template <typename T>
template <typename U>
void Vector<T>::PrintAllCenters() const {
  for (size_t i = 0; i < sz_; ++i) {
    std::cout << "Центр фигуры " << (i + 1) << ": " << arr_[i]->GeomCenter()
              << std::endl;
  }
}

template <typename T>
template <typename U>
void Vector<T>::PrintAllAreas() const {
  for (size_t i = 0; i < sz_; ++i) {
    std::cout << "Площадь фигуры " << (i + 1) << ": " << arr_[i]->Area()
              << std::endl;
  }
}

template <typename T> Vector<T>::~Vector() = default;
} // namespace vector