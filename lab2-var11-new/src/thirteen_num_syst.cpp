...
Thirteen::~Thirteen() noexcept { arr_.Clear(); }
...
Thirteen Thirteen::Sub(const Thirteen &other1, const Thirteen &other2) {
  if (other1.LessThen(other2)) {
    throw exception::NegativeException(
        "Negative answer: first number shouldn't be less than second.");
  }
  ...
  for (size_t i = 0; i < result.arr_.Size(); ++i) {
    ...
    int8_t second =
        (i < other2.arr_.Size()) ? static_cast<int8_t>(other2.arr_.Get(i)) : 0;
    first -= bor;
    if (first < second) {
...
void Thirteen::Copy(const Thirteen &other) { arr_.Copy(other.arr_); }
